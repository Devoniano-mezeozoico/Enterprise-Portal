...
def listar_noticias(limite=None):
    ...
    cursor = conn.cursor()
    sql = "SELECT * FROM noticias WHERE excluido_em IS NULL ORDER BY criado_em DESC"
    params = ()
    if limite:
        sql += " LIMIT ?"
        params = (limite,)
...
def buscar_noticia_por_id(nid):
    ...
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM noticias WHERE id=? AND excluido_em IS NULL", (nid,))
    n = cursor.fetchone()
    conn.close()
    return n
...
def excluir_noticia(nid):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("UPDATE noticias SET excluido_em=CURRENT_TIMESTAMP WHERE id=? AND excluido_em IS NULL", (nid,))
    conn.commit()
    conn.close()
...
def contar_noticias():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM noticias WHERE excluido_em IS NULL")
    t = cursor.fetchone()[0]
    conn.close()
    return t
...
def buscar_noticias_relevantes(pergunta, limite=2):
    ...
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM noticias WHERE excluido_em IS NULL")
    ns = cursor.fetchall()
    conn.close()
    pontuadas = []
    for n in ns:
