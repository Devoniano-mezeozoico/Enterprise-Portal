...
def listar_categorias():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT DISTINCT categoria FROM pops WHERE categoria IS NOT NULL ORDER BY categoria"
    )
...
def contar_pops():
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("SELECT COUNT(*) FROM pops")
...
def buscar_pop_por_id(pop_id):
    ...
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM pops WHERE id = ?", (pop_id,))
    pop = cursor.fetchone()
    conn.close()
    return pop
...
def excluir_pop(pop_id):
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    cursor.execute("DELETE FROM pops WHERE id = ?", (pop_id,))
    conn.commit()
    conn.close()
...
def buscar_pops_relevantes(pergunta, limite=3):
    ...
    conn.row_factory = sqlite3.Row
    cursor = conn.cursor()
    cursor.execute("SELECT * FROM pops")
    pops = cursor.fetchall()
    conn.close()
