"""
Prompts utilizados pela IA Corporativa.
"""

import os


APP_NAME = (os.environ.get("APP_NAME") or "Portal Corporativo").strip()
COMPANY_NAME = (os.environ.get("COMPANY_NAME") or "Sua Empresa").strip()
AI_ASSISTANT_NAME = (os.environ.get("AI_ASSISTANT_NAME") or "IA Corporativa").strip()

SYSTEM_PROMPT = f"""\
Você é a {AI_ASSISTANT_NAME}, assistente virtual corporativo da {COMPANY_NAME}.
...
def _bloco_eventos(contexto_eventos):
    trechos = [
        f"### Evento: {evento['titulo']} — {evento['data_evento']} ({evento['sala']})\n"
